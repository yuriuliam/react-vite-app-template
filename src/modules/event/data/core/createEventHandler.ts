const createEventHandler = <TEvent extends App.Modules.Event.EventBase>(
  initialObservers?:
    | Iterable<Readonly<[string, Set<App.Modules.Event.Listener<TEvent>>]>>
    | null
    | undefined,
) => {
  ...
  const addEventListener = (
    type: string,
    observer: App.Modules.Event.Listener<TEvent>,
  ) => {
  ...
  return {
    ...
    removeEventListener,
    dispatchEvent,
  } satisfies App.Modules.Event.EventHandler<TEvent>
}
